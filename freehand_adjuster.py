...
from qgis.PyQt.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                                 ...
                                 QCheckBox, QGraphicsSimpleTextItem, QMessageBox,
                                 QComboBox)
...
class LengthInputDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        header.setToolTip("Adjust lengths of sides connected to the selected vertex")
        ...
        for side in self.sides:
            ...
            spinbox.setToolTip(f"Set length for {side['name']} in {self.current_unit}")
            side['input'] = spinbox
            self.original_lengths[side['name']] = length_in_unit
            form.addRow(f"{side['name']}:", spinbox)
        layout.addLayout(form)
        if len(self.sides) == 2:
            self.auto_adjust_cb = QCheckBox("Auto-adjust other side to maintain valid triangle")
            self.auto_adjust_cb.setChecked(False)
            self.auto_adjust_cb.setToolTip("Automatically adjust one side to ensure triangle inequality")
            layout.addWidget(self.auto_adjust_cb)
        self.preview_cb = QCheckBox("Show preview")
        self.preview_cb.setChecked(True)
        self.preview_cb.setToolTip("Toggle real-time preview of vertex adjustments")
    ...
    def _on_unit_changed(self):
        ...
        for side in self.sides:
            ...
            try:
                new_value = convert_length(current_value, self.current_unit, new_unit)
        ...
        try:
            ...
            if len(self.sides) == 1:
                ...
                if not triangle_valid:
                    if self.auto_adjust_cb.isChecked():
                        self.status_label.setText("✓ Auto-adjusted for valid triangle")
                    else:
            ...
            if new_vertices:
                ...
                if self.status_label.text() == "":
                    self.status_label.setText("⚠ Cannot calculate new position")
        except Exception as e:
            self._clear_preview()
    ...
    def _get_new_lengths(self):
        ...
        for side in self.sides:
            try:
                meters_value = convert_length(side['input'].value(), self.current_unit, 'metres')
...
class VertexTool(QgsMapTool):
    ...
    def _init_rubber_bands(self):
        self.vertex_rubber_band = QgsRubberBand(self.canvas, QgsWkbTypes.PointGeometry)
    ...
    def activate(self):
        ...
        if not self._check_layer():
            return
        topo_enabled = QgsProject.instance().topologicalEditing() and QgsProject.instance().snappingConfig().enabled()
    ...
    def _check_layer(self):
        ...
        if isinstance(layer, QgsRasterLayer):
            iface.messageBar().pushMessage("Error", "Raster layers not supported", level=1, duration=1)
            return False
        if not isinstance(layer, QgsVectorLayer):
            iface.messageBar().pushMessage("Error", "Select a vector layer", level=1, duration=1)
            return False
        if layer.geometryType() == QgsWkbTypes.PointGeometry:
            iface.messageBar().pushMessage("Error", "Point layers not supported", level=1, duration=1)
            return False
        if layer.geometryType() not in (QgsWkbTypes.PolygonGeometry, QgsWkbTypes.LineGeometry):
            iface.messageBar().pushMessage("Error", "Line or polygon layer required", level=1, duration=1)
            return False
        if layer != self.layer:
            self._reset()
            self.layer = layer
    ...
    def _show_length_labels(self):
        ...
        for i in range(segments):
            start = vertices[i]
            end = vertices[(i + 1) % len(vertices)] if is_polygon else vertices[i + 1]
            length_meters = self.distance_area.measureLine(start, end)
            length_in_unit = length_meters / unit_info['factor']
            mid = QgsPointXY((start.x() + end.x()) / 2, (start.y() + end.y()) / 2)
        ...
        try:
            if geom_type == QgsWkbTypes.PolygonGeometry:
                ...
                if self.geometry.isMultipart():
                    ...
                    vertices.extend(QgsPointXY(p) for p in self.geometry.asPolyline())
        except:
            pass
        return vertices
    ...
    def _get_connected_sides(self, vertex_idx):
        ...
        if is_line:
            if vertex_idx > 0:
                ...
                length = self.distance_area.measureLine(vertices[prev_idx], vertices[vertex_idx])
                sides.append({
            ...
            if vertex_idx < len(vertices) - 1:
                ...
                length = self.distance_area.measureLine(vertices[vertex_idx], vertices[next_idx])
                sides.append({
            ...
            prev_length = self.distance_area.measureLine(vertices[prev_idx], vertices[vertex_idx])
            sides.append({
                'name': f'Side {prev_idx}-{vertex_idx}',
                'length': prev_length,
                'start_vertex': prev_idx,
            ...
            next_length = self.distance_area.measureLine(vertices[vertex_idx], vertices[next_idx])
            sides.append({
                'name': f'Side {vertex_idx}-{next_idx}',
                'length': next_length,
                'start_vertex': vertex_idx,
    ...
    def _show_length_dialog(self, vertex_idx):
        ...
        if not sides:
            return
        dialog = LengthInputDialog(vertex_idx, sides, self, self.current_unit, iface.mainWindow())
    ...
    def _apply_length_changes(self, vertex_idx, sides, new_lengths):
        try:
            ...
            if len(sides) == 1:
                ...
                new_vertices = self._adjust_two_sides(vertices, vertex_idx, sides, new_lengths)
            if new_vertices:
                ...
                iface.messageBar().pushMessage("Error", "Could not apply changes", level=1, duration=1)
        except Exception as e:
    ...
    def _adjust_single_side(self, vertices, vertex_idx, side, new_length):
        ...
        try:
            ...
            iface.messageBar().pushMessage("Error", f"Single side adjustment failed: {str(e)}", level=1, duration=1)
            return None
    ...
    def _adjust_two_sides(self, vertices, vertex_idx, sides, new_lengths):
        ...
        try:
            ...
            base_length = self.distance_area.measureLine(prev_vertex, next_vertex)
            if base_length == 0:
                raise ValueError("Cannot adjust: base vertices are coincident")
            cos_angle = (target_prev**2 + base_length**2 - target_next**2) / (2 * target_prev * base_length)
            ...
            current = vertices[vertex_idx]
            new_vertices[vertex_idx] = pos1 if current.distance(pos1) <= current.distance(pos2) else pos2
            return new_vertices
        except Exception as e:
            iface.messageBar().pushMessage("Error", f"Two-side adjustment failed: {str(e)}", level=1, duration=1)
            return None
    ...
    def _move_vertex(self, vertex_idx, new_position):
        ...
        try:
            ...
            topological_editing = QgsProject.instance().topologicalEditing() and QgsProject.instance().snappingConfig().enabled()
            ...
            iface.messageBar().pushMessage("Error", f"Vertex move failed: {str(e)}", level=1, duration=1)
    ...
    def _get_feature_vertices(self, feature):
        ...
        try:
            ...
            iface.messageBar().pushMessage("Warning", f"Failed to extract vertices: {str(e)}", level=1, duration=1)
        return vertices
    ...
    def _update_length_labels(self, preview_vertices=None):
        ...
        for i, label in enumerate(self.length_labels[:segments]):
            ...
            canvas_point = self.toCanvasCoordinates(mid)
            label.setText(f"{length_in_unit:.{unit_info['decimals']}f}{unit_info['suffix']}")
            label.setPos(canvas_point.x() - label.boundingRect().width() / 2,
                        canvas_point.y() - label.boundingRect().height() / 2)
