...
from PyQt5.QtWidgets import (
    ...
    QLabel, QDoubleSpinBox, QMessageBox, QGraphicsTextItem
)
...
class UnifiedPolygonEditTool(QgsMapTool):
    ...
    def showLengthPanel(self):
        ...
        if dlg.exec_():
            ...
            if self.mode == 'side' and length <= 0:
                iface.messageBar().pushMessage("Error", "Side length must be positive.", duration=2)
                self.resetTool()
                return
    ...
    def adjustSideLength(self, fixed_vertex_index, moving_vertex_index, target_length):
        ...
        # Remove closing duplicate if polygon closed
        if vertices[0] == vertices[-1]:
            vertices = vertices[:-1]
        ...
        if current_length < 1e-10:
            iface.messageBar().pushMessage(
                "Error", "Invalid side length (side too short).", duration=2)
            self.resetTool()
            return
    ...
    def moveSideVertex(self, vertex_index, new_point):
        try:
            ...
            self.update_dimension_labels()
            iface.messageBar().pushMessage(
                "Success", f"Side length set to {self.targetLength:.3f} units!", duration=2)
        except Exception as e:
            iface.messageBar().pushMessage(
    ...
    def update_dimension_labels(self):
        ...
        for i in range(n):
            ...
            txt_item = BufferedTextItem(f"{length:.2f}", main_color=QColor(
                255, 255, 255), buffer_color=QColor('black'), buffer_width=3)
            txt_item.setPos(scene_pt.x() - 15, scene_pt.y() - 10)
