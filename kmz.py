# -*- coding: utf-8 -*-
"""
KMZ Exporter – Modular, maintainable plugin for QGIS 3.28+
...
class QtCompat:
    ...
    def palette_role(role_name):
        ...
        roles = {
            ...
            'window_text': QPalette.WindowText,      # ✅ Fixed: was "Windowtext"
            'base': QPalette.Base,
    ...
    def non_modal():
        if hasattr(Qt, 'WindowModality'):
            ...
            return 0  # Fallback for Qt.NonModal
    ...
    def _update_display(self):
        if self.show_alpha:
            ...
            self.btn_preview.setStyleSheet(
                ...
                f"{self.current_color.green()}, {self.current_color.blue()}, {alpha / 255.0}); border: 1px solid #999;"
            )
        else:
            self.btn_preview.setStyleSheet(
...
class FeatureProcessor:
    ...
    def get_label_text(self, feature, layer):
        if layer.id() in self.label_field_map:
            fn = self.label_field_map[layer.id()]
    ...
    def create_description_html(self, feature, label=None):
        html_parts = []
        if label:
        ...
        for idx, fld in enumerate(feature.fields()):
            ...
            html_parts.append(
                ...
                f"<td style='padding:8px;font-weight:bold;border:1px solid #ddd;color:#555;word-wrap:break-word;'>{safe_name}</td>"
                f"<td style='padding:8px;border:1px solid #ddd;word-wrap:break-word;'>{display_val}</td></tr>"
            )
...
class KMZExporter:
    ...
    def set_label_fields(self, d):
        self.label_field_map = d

    def set_symbology_settings(self, d):
        self.symbology_settings = d

    def set_label_settings(self, d):
        self.label_settings = d
    ...
    def _export_vector_layer(self, layer, doc, label_mode, selected_only):
        ...
        processor = FeatureProcessor(self.label_field_map)
    ...
    def _export_feature(self, feature, folder, style_id, transform, layer, label_mode, processor):
        ...
        if label_mode == 'description':
            desc = processor.create_description_html(feature, label=label)
        elif label_mode == 'both':
            desc = processor.create_description_html(feature, label=label)
        else:
            desc = processor.create_description_html(feature)
...
class ExportThread(QThread):
    ...
    def __init__(self, layers, out_path, label_mode_map, label_fields, sym_settings, lbl_settings, selected_only):
        super().__init__()
        self.layers = layers
...
class SymbologyWidget:
    ...
    def create_point_widget(save_callback):
        ...
        cb_icon = QComboBox()
        icons = [
            ("Yellow pushpin", "http://maps.google.com/mapfiles/kml/pushpin/ylw-pushpin.png"),
            ("Red pushpin", "http://maps.google.com/mapfiles/kml/pushpin/red-pushpin.png"),
            ("Blue pushpin", "http://maps.google.com/mapfiles/kml/pushpin/blue-pushpin.png"),
            ("Green pushpin", "http://maps.google.com/mapfiles/kml/pushpin/grn-pushpin.png")
        ]
...
class KMZExporterDialog(QDialog):
    ...
    def _setup_style(self):
        ...
        colors = {
            ...
            'alternate_base': QColor(237, 240, 244),
            'text': QColor(33, 37, 41),
            'button': QColor(230, 234, 239),
            'button_text': QColor(33, 37, 41),
            'highlight': QColor(0, 123, 255),
    ...
    def _create_layer_selection_group(self):
        ...
        self.lst_layers.itemClicked.connect(self._on_layer_clicked)
    ...
    def _create_settings_group(self):
        ...
        self.settings_layer_combo.setMinimumWidth(250)
        ...
        tabs.addTab(self._create_label_tab(), "Label")
    ...
    def _create_label_tab(self):
        ...
        layout.addWidget(QLabel("Label display:"))
        grid = QGridLayout()
        self.r_name = QRadioButton("Show on map")
        self.r_desc = QRadioButton("Show in popup only")
        self.r_both = QRadioButton("Show on map + popup")
        self.r_none = QRadioButton("No labels")
        ...
        h = QHBoxLayout()
        h.addWidget(QLabel("Label field:"))
        self.cb_label_fld = QComboBox()
        self.cb_label_fld.setToolTip("Choose a field to use as label")
        self.cb_label_fld.currentIndexChanged.connect(self._save_label_field)
        h.addWidget(self.cb_label_fld, 1)
        layout.addLayout(h)

        layout.addWidget(QLabel("Label style:"))
        h = QHBoxLayout()
        h.addWidget(QLabel("Size:"))
        self.sp_lbl_scale = QDoubleSpinBox()
        self.sp_lbl_scale.setRange(0.5, 3.0)
        ...
        layout.addWidget(self.picker_lbl)
    ...
    def _load_layers(self):
        ...
        for layer_id, layer in project.mapLayers().items():
            ...
            self.settings_layer_combo.addItem(layer.name(), layer_id)
            self.label_mode_map[layer_id] = 'name'
        if self.settings_layer_combo.count():
            self._on_settings_layer_changed()
    ...
    def _on_layer_clicked(self, item):
        item.setCheckState(QtCompat.checkstate(
            item.checkState() == QtCompat.checkstate(False)))

    def _on_settings_layer_changed(self):
        self._update_label_field_combo()
    ...
    def _update_label_field_combo(self):
        self.cb_label_fld.blockSignals(True)
        self.cb_label_fld.clear()
        layer_id = self.settings_layer_combo.currentData()
        if layer_id and layer_id in self.layers_dict:
            layer = self.layers_dict[layer_id]
            for field in layer.fields():
                self.cb_label_fld.addItem(field.name(), field.name())
            field_name = self.label_field_map.get(layer_id)
            if field_name:
                idx = self.cb_label_fld.findData(field_name)
                if idx >= 0:
                    self.cb_label_fld.setCurrentIndex(idx)
        self.cb_label_fld.blockSignals(False)

    def _save_label_field(self):
        layer_id = self.settings_layer_combo.currentData()
        if layer_id:
            field = self.cb_label_fld.currentData()
            if field:
                self.label_field_map[layer_id] = field
            else:
                self.label_field_map.pop(layer_id, None)
    ...
    def _save_label_style(self):
        ...
        if layer_id:
            self.label_settings[layer_id] = {
                'scale': self.sp_lbl_scale.value(),
                'color': self.picker_lbl.get_color()
            }
    ...
    def _save_symbology(self):
        ...
        if not settings['use_qgis']:
            if geom_type == QgsWkbTypes.PointGeometry:
                settings.update({
                    'icon_url': self.w_pt.cb_icon.currentData(),
                    'size': self.w_pt.sp_size.value(),
                    'color': self.w_pt.picker.get_color()
                })
            elif geom_type == QgsWkbTypes.LineGeometry:
                settings.update({
                    'width': self.w_ln.sp_width.value(),
                    'color': self.w_ln.picker.get_color()
                })
            elif geom_type == QgsWkbTypes.PolygonGeometry:
                settings.update({
                    'fill_color': self.w_poly.picker_fill.get_color(),
                    'outline_color': self.w_poly.picker_outline.get_color(),
                    'outline_width': self.w_poly.sp_width.value()
                })
    ...
    def _start_export(self):
        ...
        self.thread = ExportThread(
            selected_layers, self.output_path, self.label_mode_map,
            self.label_field_map, self.symbology_settings, self.label_settings,
            self.chk_selected_only.isChecked()
        )
